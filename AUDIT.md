...
An earlier audit brief recorded questions posed before the v7 and v14 audits. This file exists so
that the audit is adversarial and specific rather than a re-read. It is a briefing, not a status
report — `NEXT.md` is the plan and `LEARNINGS.md` is the narrative.
...
`gpt-5.4-nano` and `deepseek-flash` on the frozen 81-case legacy/development routing benchmark and
27 scripted calls.
...
**Why to distrust it.** At the time of this brief, the severity file had been trimmed for held-out
echoes (1736 → 1645 rows) and was being extended. Treat those counts as historical; check the
current file and manifest before comparing any two runs.
...
- No GPU is needed for the local demo; the model runs through `laya-mlx` on Apple Silicon.
- `uv run pytest -q` (171 tests) and `uv run python scripts/eval_severity.py` are the entry points.
- API keys belong in `.env` for the LLM arms. **Do not spend on generation or labelling without
