...
BASE_MODEL_ID = str(RUN_CONFIG["base_model_id"])
BASE_MODEL_REVISION = str(RUN_CONFIG["base_model_revision"])
...
MD_TRAIN = f"""## 3. Fine-tune (RLCD, DDP across both T4s)

The maintainer's training script, vendored verbatim at `training/train_ddp.py`. This recipe runs
{EPOCHS} epochs; expect roughly 5–20 minutes depending on dataset size.
"""
...
INSTALL = '''# Primary versions match the recorded v7 Kaggle run. Kaggle supplies the CUDA-enabled torch build.
...
import os
MODEL_REVISION = os.environ.get("LAYA_MODEL_REVISION", "{BASE_MODEL_REVISION}")
